<template>
  <div>
    <div class="page-heading">
      <div>
        <h1>上传活动照片</h1>
        <p>使用 Element Plus Upload 选择图片，提交后写入 json-server 相册数据。</p>
      </div>
      <el-button @click="$router.push(`/albums/${activityId}`)">返回相册</el-button>
    </div>

    <el-card shadow="never" class="upload-card">
      <el-form label-position="top" :model="form">
        <el-form-item label="照片标题">
          <el-input v-model="form.title" maxlength="30" show-word-limit placeholder="例如：活动现场合影" />
        </el-form-item>
        <el-form-item label="选择照片">
          ...
          >
            ...
            <template #tip>
              <div class="el-upload__tip">演示环境会把图片转成 Base64 并保存到 db.json。</div>
            </template>
          </el-upload>
        </el-form-item>
